    ...
    public ResponseEntity<?> courseSchedule(@RequestParam(value = "course_code", required = false) String courseCode,
        ...
        try {
            ...
            if ((code == null || code.isBlank()) && title != null && !title.isBlank()) {
                ...
                if (found.isEmpty()) return ResponseEntity.ok(Map.of("found", false));
                code = found.get();
            }
            if (code == null || code.isBlank()) return ResponseEntity.badRequest().body(Map.of("error", "provide course_code or title"));

            var rangeOpt = sql.queryCourseDateRange(code);
            if (rangeOpt.isEmpty()) return ResponseEntity.ok(Map.of("found", false, "course_code", code));
