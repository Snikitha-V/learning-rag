 ...
 * 
 * Customers select LLM via LLM_PROVIDER environment variable:
 *   - "llama" (default) → LlamaCppProvider (local llama.cpp server)
 *   - "openai" → OpenAIProvider (OpenAI API)
 *   - "custom_http" → CustomHttpProvider (customer's own LLM API)
 */
public class LLMFactory {
    ...
    public static LLMProvider createProvider() {
        String provider = Config.LLM_PROVIDER.toLowerCase();
